...
void createScene(ExtendedController& c, ShaderIF* sIF)
{
	...
	Building* b2 = new Building(sIF, 10.0, 1.0, 1.0, p[4]);
	Building* b3 = new Building(sIF, 20.0, 1.0, 1.0, p[2]);
	Building* b4 = new Building(sIF, 30.0, 1.0, 1.0, p[3]);
	Building* b5 = new Building(sIF, -10.0, 1.0, 1.0, p[4]);
	Building* b6 = new Building(sIF, -20.0, 1.0, 1.0, p[3]);
	Building* b7 = new Building(sIF, -30.0, 1.0, 1.0, p[2]);

	Block* green = new Block(sIF, -35.0, 0.5, 0.0, 75.0, 0.5, 10.0, p[2], 1);
	Block* road = new Block(sIF, -35.0, 0.5, 10.0, 75.0, 0.5, 15.0, p[1]);
	Block* green2 = new Block(sIF, -35.0, 0.5, 25.0, 75.0, 0.5, 10.0, p[2], 1);
	...
	c.addModel(new Truck(sIF, 0.0, 1.25, 13.0, p[4]));
	c.addModel(new Truck(sIF, -22.0, 1.25, 21.0, p[3]));

	Tree* t1 = new Tree(sIF, 5.0, 1.0, 31.0);
	c.addModel(t1);
...
{
	ExtendedController c("--Project 4: A Colorful Street Part 3: The Final Showdown--", MVC_USE_DEPTH_BIT);
	c.reportVersions(std::cout);

		ShaderIF::ShaderSpec glslProg[] =
	{
	    { "shaders/basic.vsh", GL_VERTEX_SHADER },
	    { "shaders/shrinkTriangles.gsh", GL_GEOMETRY_SHADER },
