...
Truck::Truck(ShaderIF* sIF, float cx, float cy, float cz, PhongMaterial& p) : SceneElement(sIF)
{
	xmin = cx;
    xmax = cx+3.0;
    ymin = cy;
	...
	blocks[0] = new Block(sIF, cx, cy, cz, 1.0, 1.0, 2.0, p);
	blocks[1] = new Block(sIF, cx+1.0, cy, cz, 2.0, 2.0, 2.0, p);
	blocks[2] = new Block(sIF, cx+0.8, 1.4*cy, cz+0.1, 0.5, 1.3, 1.8, window);
	blocks[3] = new Block(sIF, cx-0.1, cy+0.2, cz, 0.1, 0.5, 0.5, light);
	blocks[4] = new Block(sIF, cx-0.1, cy+0.2, cz+1.5, 0.1, 0.5, 0.5, light);
	blocks[5] = new Block(sIF, cx+3.0, cy+0.3, cz, 8.0, 2.5, 2.0, white);
	...
	cyls[4] = new Cylinder(sIF, (double)cz-.25, (double)cz+0.75, (double)cy, (double)cx+3.5, 0.4, rubber);
	cyls[5] = new Cylinder(sIF, (double)cz+2.25, (double)cz+2.0, (double)cy, (double)cx+3.5, 0.4, rubber);
	cyls[6] = new Cylinder(sIF, (double)cz-.25, (double)cz+0.75, (double)cy, (double)cx+9.5, 0.4, rubber);
	cyls[7] = new Cylinder(sIF, (double)cz+2.25, (double)cz+2.0, (double)cy, (double)cx+9.5, 0.4, rubber);
	cyls[8] = new Cylinder(sIF, (double)cz-.25, (double)cz+0.75, (double)cy, (double)cx+10.5, 0.4, rubber);
	cyls[9] = new Cylinder(sIF, (double)cz+2.25, (double)cz+2.0, (double)cy, (double)cx+10.5, 0.4, rubber);
...
void Truck::getMCBoundingBox(double* xyzLimits) const
{
	xyzLimits[0] = xmin;
	xyzLimits[1] = xmax;
	xyzLimits[2] = ymin;
