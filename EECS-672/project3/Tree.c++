...
Tree::Tree(ShaderIF* sIF, float cx, float cy, float cz) : SceneElement(sIF)
{
	...
	block = new Block(sIF, cx+0.5, cy, cz+0.5, 0.3, 4.0, 0.3, wood);
	cyls[0] = new Cylinder(sIF, cz, cz+1.0, cy+3.0, cx, 1.0, leaves);
	cyls[1] = new Cylinder(sIF, cz+0.1, cz+1.1, cy+3.0, cx+1.0, 1.0, leaves);
	cyls[2] = new Cylinder(sIF, cz-0.1, cz+0.9, cy+3.5, cx+0.5, 1.0, leaves);
}
